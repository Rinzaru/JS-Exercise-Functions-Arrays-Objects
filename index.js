...
function temperatureCtoF(celNum) {
  let cToF = (celNum * 9) / 5 + 32;
 ...
 * }
 */
function makePersonObject(first, second, third) {
  /* code here */
 ...
 */
function getName(objArg) {
  /* code here */

  return "my name is " + objArg.name;
}

// console.log(getName(makePersonObject(1, "Luke", "leia@leia.com")));
 ...
 */
function isItAnApple(checkArray) {
  /* code here */
  const newArray = [];
  if (Array.isArray(checkArray) === false) {
    return "Not an array";
  } else {
    for (let i = 0; i < checkArray.length; i++) {
      if (checkArray[i] !== "apple") {
        newArray.push(false);
      } else {
        newArray.push(true);
      }
...
function getCarInfoByIndex(inventory, index) {
  ...
  let cMake = inventory[index].car_make;
  let cModel = inventory[index].car_model;
  return `This is a ${cMake} ${cModel}`;
}

console.log(getCarInfoByIndex(items, 0));
...
console.log(getLastCarInfo(items));
/**
...
console.log(getModelYears(items));
/**
